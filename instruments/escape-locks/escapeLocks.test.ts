...
describe('cerraduras de sostener', () => {
  ...
  it('la posición y el imán también, y soltar pierde lo acumulado', () => {
    ...
    expect(advanceLock({ kind: 'pose', pose: 'upright' }, halfway, reading({ pose: 'face-up' }), 0.1).heldSeconds).toBe(
      0,
    );
  });
  ...
  it('la barra de progreso va de 0 a 1', () => {
    ...
    expect(lockCompletion(noteLock, holdFor(noteLock, reading({ noteCentsError: 0 }), 0.6))).toBeCloseTo(0.4, 6);
  });
});
...
describe('cerradura de golpes', () => {
  ...
  it('se abre con tres golpes seguidos, al terminar la serie', () => {
    let lockProgress = advanceLock(
      knockLock,
      startLockProgress(),
      reading({ newKnockTimesSeconds: [0, 0.4, 0.8], nowSeconds: 0.9 }),
      0.1,
    );
  ...
  it('cuatro golpes no valen, y lo dice', () => {
    let lockProgress = advanceLock(
      knockLock,
      startLockProgress(),
      reading({ newKnockTimesSeconds: [0, 0.3, 0.6, 0.9], nowSeconds: 1 }),
      0.1,
    );
  ...
  it('una pausa larga empieza la cuenta de nuevo', () => {
    let lockProgress = advanceLock(
      knockLock,
      startLockProgress(),
      reading({ newKnockTimesSeconds: [0, 0.4], nowSeconds: 0.5 }),
      0.1,
    );
    lockProgress = advanceLock(
      knockLock,
      lockProgress,
      reading({ newKnockTimesSeconds: [2, 2.4, 2.8], nowSeconds: 2.9 }),
      0.1,
    );
    lockProgress = advanceLock(knockLock, lockProgress, reading({ nowSeconds: 4.4 }), 0.1);
    expect(lockProgress.isOpen).toBe(true);
  });
});
...
describe('parseStoredPuzzles', () => {
  it('conserva los puzles válidos y descarta cerraduras rotas', () => {
      ...
      JSON.stringify([
        {
          ...
          secret: '4721',
          locks: [
            { kind: 'note', noteIndex: 9 },
            { kind: 'pose', pose: 'volando' },
            { kind: 'knocks', knockCount: 3 },
          ],
        },
    ...
    expect(storedPuzzles[0]!.locks).toEqual([
      { kind: 'note', noteIndex: 9 },
      { kind: 'knocks', knockCount: 3 },
    ]);
